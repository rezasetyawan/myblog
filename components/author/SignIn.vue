...
<template>
  <section class="font-rubik flex justify-center items-center h-screen w-full p-3 bg-gray-100">
    <form class="min-w-[90%] sm:min-w-[60%] lg:min-w-[30%] relative" @submit.prevent="onSubmitHandler">
      ...
      <input class="focus:outline-none p-3 border-[1px] border-accent-color2 block my-4 rounded-md w-full" type="email"
        placeholder="email" required v-model="user.email" />
      <div class="relative">
        <input class="focus:outline-none p-3 border-[1px] border-accent-color2 block my-4 rounded-md w-full"
          :type="isPasswordVisible ? 'text' : 'password'" placeholder="password" required v-model="user.password" />
        <span class="absolute top-[35%] right-3 cursor-pointer" @click="isPasswordVisible = !isPasswordVisible">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" fill="#94b8a3" class="w-5"
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="#94b8a3" class="w-5"
            v-if="!isPasswordVisible">
            <path
