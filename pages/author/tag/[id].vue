...
import { getTagById, updateTagById } from "../../../composables/useTags";
const route = useRoute();
const client = useSupabaseClient();
const id = ref<string>(route.params.id as string);

const tag = ref<{ id: string; name: string } | undefined | null>();

onMounted(async () => {
  id.value = route.params.id as string;
  const data = await getTagById(client, id.value);
  tag.value = data;
});
...
const onSubmitHandler = async () => {
  try {
    if (tag.value) {
      await updateTagById(client, id.value, tag.value.name);
...
useHead({
  title: `${tag.value?.name} | My Blog`,
  titleTemplate: `${tag.value?.name} | My Blog`,
});
</script>
<template>
  <section class="flex justify-center">
    <button
      ...
      @click="() => useRouter().push('/author/tags')"
    >
